...
export default function Dashboard() {
  ...
  const maxTrend = stats?.risk_trend ? Math.max(...stats.risk_trend.map((t) => t.avg_score), 1) : 1;

  return (
    <div className="flex min-h-screen bg-[#1e1e1e] text-white font-mono selection:bg-gray-700">
      {/* LEFT SIDEBAR */}
      <div className="w-80 border-r border-gray-800 bg-[#161616] p-6 flex-col hidden md:flex overflow-y-auto">
        <h2
          className="text-3xl font-bold tracking-tight mb-8"
          style={{ fontFamily: "cursive, sans-serif" }}
        >
          PRISM{" "}
          <span className="text-sm text-gray-500 font-sans tracking-normal ml-1">v2.1</span>
        </h2>

        {/* LLM Provider Badge */}
        {stats && (
          <div className="mb-6 p-3 bg-[#1e1e1e] rounded-lg border border-gray-800">
            <div className="text-xs text-gray-500 uppercase tracking-widest mb-1">AI Engine</div>
            <div className={`text-sm font-bold ${providerBadge(stats.llm_provider).color}`}>
              {providerBadge(stats.llm_provider).icon} {providerBadge(stats.llm_provider).label}
            </div>
          </div>
        )}
          ...
          {stats?.recent_repos && stats.recent_repos.length > 0 ? (
            <ul className="space-y-3 text-sm text-gray-300">
              {stats.recent_repos.map((r) => (
                <li
                  key={r}
                  className="flex items-center gap-3 p-2 bg-[#1e1e1e] rounded-md border border-gray-800"
                >
              ...
              {issues.slice(0, 5).map((issue) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-200 leading-tight">
                    <span className="text-gray-400">{issue.repo.split("/").pop()}</span> #
                    {issue.pr_number}
                  </div>
            ...
            <div className="bg-[#161616] border border-gray-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="flex items-end gap-1 h-24">
                {stats.risk_trend.map((t) => {
                  const height = maxTrend > 0 ? (t.avg_score / maxTrend) * 100 : 0;
                  const barColor =
                    t.avg_score === 0
                      ? "bg-green-600"
                      : t.avg_score < 30
            ...
            <div className="bg-[#161616] border border-gray-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                ...
                <div className="bg-[#1e1e1e] rounded-lg p-2.5 border border-green-900/30">
                  ...
                  <div className="text-lg font-bold text-green-500">+{stats.total_lines_added}</div>
                </div>
                <div className="bg-[#1e1e1e] rounded-lg p-2.5 border border-red-900/30">
                  <div className="text-[10px] text-red-600 mb-0.5">Deleted</div>
                  <div className="text-lg font-bold text-red-500">-{stats.total_lines_deleted}</div>
                </div>
              </div>
              {/* Safe PR Rate */}
              <div className="mt-2">
                <div className="flex justify-between text-xs mb-1.5">
                  ...
                  <span className="text-green-400 font-bold">{stats.safe_pr_percentage}%</span>
                </div>
            ...
            <div className="bg-[#161616] border border-gray-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="flex gap-4 mb-3">
                <div className="bg-[#1e1e1e] rounded-lg p-2.5 border border-gray-800/50 flex-1">
                  <div className="text-[10px] text-gray-500 mb-0.5">Auto-Merged</div>
                  <div className="flex items-end gap-1.5">
                    <span className="text-xl font-bold text-purple-400">
                      {stats.auto_merged_count}
                    </span>
                    <span className="text-xs text-gray-500 mb-0.5">/ {stats.total_analyzed}</span>
                  </div>
                </div>
                <div className="flex-1">
                  <div className="text-[10px] text-gray-500 mb-1.5">Top Contributors</div>
                    ...
                    <div className="space-y-1">
                      {stats.top_authors.map((a) => (
                        ...
                        >
                          ...
                          <span className="text-blue-400 font-bold">{a.count}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-xs text-gray-600 italic">No data yet</div>
                  )}
                </div>
              </div>
              {/* Risk Categories */}
              {stats.top_categories && stats.top_categories.length > 0 && (
                <div>
                  <div className="text-[10px] text-gray-500 mb-1.5">Top Risk Categories</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3 flex-grow break-words text-[13px] font-semibold leading-snug text-white drop-shadow-md">
                      ...
                      {analysis.repo.split("/").pop()} :: PR #{analysis.pr_number} ::{" "}
                      {analysis.pr_title}
                    </div>
                    ...
                    <div className="px-3 py-1.5 bg-black/20 text-[11px] flex justify-between items-center text-gray-100 font-medium border-t border-black/10">
                      ...
                      <div className="font-bold tracking-wide">Score: {analysis.score}</div>
                    </div>
                  </a>
                );
              })
